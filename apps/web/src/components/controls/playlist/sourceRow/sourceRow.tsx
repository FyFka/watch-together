...
import styles from "./sourceRow.styles.css";
...
function SourceRow({ source, selectedSource, onSelectSource, onDeleteSource }: IVideoRowProps) {
  ...
  const isSourceSelected = source === selectedSource;
  return (
    <div className={styles.videoRow}>
      <Button onClick={handleSelectVideo} className={styles.video} disabled={isSourceSelected}>
        {source}
      </Button>
      <Button className={styles.delete} onClick={handleDeleteVideo}>
        <svg className={styles.deleteIcon} fill="none" viewBox="0 0 24 24">
          <path
            fill="#fff"
            fill-rule="evenodd"
